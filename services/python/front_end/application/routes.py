...
def start():
	...
	try:
		requests.get("http://graphing:5001/plot",timeout=5)
	except:
		pass
	try:
...
def home():
	...
	return render_template('home.html', title='Particle Swarm Optermisation', fx='z=5*sin(x/10)+5*cos(y/10)+5*sin(e^(y^2/10)+e^(x^2/10))*cos(x*y))-(0.001*(x^2+y^2)', gmax=gmax, fig_ip=os.getenv('FIG_IP'),running=run_state)